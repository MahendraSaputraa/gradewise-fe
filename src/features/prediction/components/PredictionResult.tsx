import { BrainCircuit, CheckCircle2, RotateCcw, Sparkles, TrendingUp } from 'lucide-react'
...
export function PredictionResult({ result, onReset }: PredictionResultProps) {
  ...
  return (
    <section className="result-panel score-result-panel" aria-live="polite">
      <div className="result-topline"><span>Hasil prediksi model</span><TrendingUp size={17} /></div>

      <div className="score-gauge" style={{ '--score': `${gaugeScore * 3.6}deg` } as CSSProperties}>
      ...
      {!result.is_dummy && (
        <div className="model-note">
          <span className="model-note-icon"><BrainCircuit size={19} /></span>
          <span><strong>Model ML aktif</strong> Hasil dihitung oleh model Student Performance yang telah dilatih.</span>
          <CheckCircle2 size={17} />
        </div>
      )}

      <div className="result-caption"><Sparkles size={14} /> Gunakan hasil sebagai insight pendukung, bukan satu-satunya dasar keputusan.</div>
